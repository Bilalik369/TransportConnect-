...
export const getReceivedRequests = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Erreur lors de la récupération des demandes reçues" });
  }
};
...
export const getRequestById = async (req, res) => {
  try {
    ...
    const isDriver =
      request.trip.driver._id.toString() === req.user._id.toString();
    ...
    res
      .status(500)
      .json({ message: "Erreur lors de la récupération de la demande" });
  }
};
...
export const createRequest = async (req, res) => {
  try {
    ...
    if (!trip.driver) {
      return res
        .status(400)
        .json({ message: "Le conducteur du trajet est introuvable" });
    }

    if (trip.status !== "active") {
      return res
        .status(400)
        .json({ message: "Ce trajet n'est plus disponible" });
    }

    if (trip.driver && trip.driver._id.toString() === req.user._id.toString()) {
      return res
        .status(400)
        .json({ message: "Vous ne pouvez pas demander votre propre trajet" });
    }
    ...
    res
      .status(500)
      .json({ message: "Erreur lors de la création de la demande" });
  }
};
...
export const acceptRequest = async (req, res) => {
  try {
    ...
    if (request.status !== "pending") {
      return res
        .status(400)
        .json({ message: "Cette demande ne peut plus être acceptée" });
    }
    ...
    res
      .status(500)
      .json({ message: "Erreur lors de l'acceptation de la demande" });
  }
};
...
export const rejectRequest = async (req, res) => {
  try {
    ...
    if (request.trip.driver.toString() !== req.user._id.toString()) {
      return res
        .status(403)
        .json({ message: "Non autorisé à refuser cette demande" });
    }

    if (request.status !== "pending") {
      return res
        .status(400)
        .json({ message: "Cette demande ne peut plus être refusée" });
    }
...
export const cancelRequest = async (req, res) => {
  try {
    ...
    if (request.sender.toString() !== req.user._id.toString()) {
      return res
        .status(403)
        .json({ message: "Non autorisé à annuler cette demande" });
    }

    if (!["pending", "accepted"].includes(request.status)) {
      return res
        .status(400)
        .json({ message: "Cette demande ne peut plus être annulée" });
    }
  ...
  try {
    ...
    if (request.status !== "accepted") {
      return res
        .status(400)
        .json({
          message: "La demande doit être acceptée pour confirmer la collecte",
        });
    }
    ...
    res
      .status(500)
      .json({ message: "Erreur lors de la confirmation de collecte" });
  }
};
...
export const confirmDelivery = async (req, res) => {
  try {
    ...
    if (request.trip.driver._id.toString() !== req.user._id.toString()) {
      return res
        .status(403)
        .json({ message: "Seul le conducteur peut confirmer la livraison" });
    }
    ...
    if (request.status !== "in_transit") {
      return res
        .status(400)
        .json({ message: "Cette demande n'est pas en transit" });
    }
    ...
    try {
      await sendNotificationEmail(
        request.sender.email,
        "Livraison confirmée",
        `Votre colis a été livré avec succès. Merci d'avoir utilisé TransportConnect !`,
      );
    } catch (emailError) {
      console.error("Erreur envoi email:", emailError);
    }

    res.json({ request, message: "Livraison confirmée avec succès" });
  } catch (error) {
    console.error("Erreur confirmation livraison:", error);
    res
      .status(500)
      .json({ message: "Erreur lors de la confirmation de livraison" });
  }
};

export const getRequestsWithChats = async (req, res) => {
  try {
    ...
    const chats = await Chat.find({
      ...
      .populate({
        path: "request",
        select: "departureCity destinationCity departureDate itemType status",
      })
      .populate({
        path: "participants",
...
export const getRequestChat = async (req, res) => {
  try {
